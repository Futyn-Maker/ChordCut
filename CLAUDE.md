...
- **Settings** (`settings.py`) — JSON file persistence for user preferences (volume, seek step, sort orders, check for updates, active server, etc.). Unknown keys in `settings.json` are silently ignored on load; missing keys fall back to `_DEFAULTS`.

**Sorting** is per section: View → Sorting holds a Tracks submenu (`Settings.track_sort`, default newest first) and a Playlists submenu (`Settings.playlist_sort`, default oldest first), both built by `MainWindow._build_sort_submenu` from the same four keys (`alpha_asc`, `alpha_desc`, `date_desc`, `date_asc`) and applied in SQL by `Database.get_all_tracks` / `get_all_playlists`. Jellyfin has no API for ordering playlists among themselves (only for moving items inside one), so playlist order is purely a client-side sort over the cached `date_created` / name.
...
Playlist tracks can be reordered with Alt+Up/Down (one step), Alt+Home/End (to the top/bottom, `_move_playlist_item_to_edge`), the context menu, or mouse drag-and-drop (`EVT_ROW_MOVED` → `_on_row_moved`). Every path ends in `_commit_playlist_move`, which refreshes the list, updates the SQLite cache and sends exactly one `Move` API request. Dragging is enabled only in an unfiltered, unshuffled playlist view, where list indices equal playlist positions. The selection list supports the same reorder paths; its drag is always enabled (`_on_selection_row_moved`, local only). The mouse's Back button (`EVT_MOUSE_AUX1_DOWN` on the panel, lists, and lyrics panel) triggers `_go_back`, mirroring Backspace.
